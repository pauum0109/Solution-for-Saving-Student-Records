...
public class AdminAddStudentController implements Initializable {
    ...
    private void insertStudentRecord(String userID){
        int id = getMaxStudentId() + 1;
        String sql = "INSERT INTO student (studentID, studentFirstname, studentLastname, studentGender, studentDOB, studentEmail, studentMajor, studentAddress, studentPhone, userID) VALUES (?,?,?,?,?,?,?,?,?,?)";
        try{
            preparedStatement = con.prepareStatement(sql);
    ...
    void handleAddButton(ActionEvent event) {
        if(event.getSource() == btnSave){
            ...
            insertStudentRecord(insertUserRecord());
            fetRowList();
            ClearField();
        }
    }
